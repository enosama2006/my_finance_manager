...
export function ExpenseCategories() {
  ...
  const submit = (event: FormEvent) => {
    ...
    try {
      if (editingId) {
        ...
        setMessage('تم تحديث بند الصرف دون تغيير الحركات التاريخية المرتبطة به.')
      } else {
        finance.createExpenseCategory({ name, parentId: parentId || undefined, description: description || undefined })
        setMessage('تم إنشاء بند الصرف.')
      }
      clearForm()
    } catch (error) {
      setMessage(error instanceof Error ? error.message : 'تعذر حفظ بند الصرف')
    }
  }
  ...
  const archive = (category: ExpenseCategory) => {
    ...
    try {
      ...
      setMessage('تمت أرشفة البند مع الحفاظ على التاريخ.')
    } catch (error) {
      setMessage(error instanceof Error ? error.message : 'تعذر أرشفة البند')
    }
  }

  return <div className="page-stack">
    <section className="section-intro">
      <div><span className="eyebrow">EXPENSE TAXONOMY / WHAT</span><h2>بنود الصرف</h2><p>أنشئ شجرة التصنيف التي تناسبك: سكن ← خدمات ← كهرباء، أو سيارات ← وقود، أو صحة ← عيادات. بند الصرف يصف «على ماذا صُرف المال؟» ولا يحل محل المحفظة.</p></div>
    </section>

    <section className="expense-admin-grid">
      <div className="panel category-editor">
        <div className="panel-head"><div><span>{editing ? 'تعديل بند موجود' : 'بند جديد'}</span><h2>{editing ? editing.name : 'إضافة إلى الشجرة'}</h2></div><Plus size={18} /></div>
        <form className="trade-form" onSubmit={submit}>
          <label><span>اسم البند</span><input value={name} onChange={e => setName(e.target.value)} placeholder="مثال: كهرباء" /></label>
          <label><span>البند الأب</span><select value={parentId} onChange={e => setParentId(e.target.value)}><option value="">بند رئيسي</option>{active.filter(c => c.id !== editingId).map(c => <option key={c.id} value={c.id}>{depthLabel(active, c)}</option>)}</select></label>
          ...
          <div className="category-form-actions"><button className="primary" type="submit">{editing ? 'حفظ التعديل' : 'إضافة البند'}</button>{editing && <button className="ghost" type="button" onClick={clearForm}>إلغاء</button>}</div>
        </form>
        {message && <div className="operation-message ok">{message}</div>}
      </div>

      <div className="panel category-tree-panel">
        <div className="panel-head"><div><span>الدليل الشجري</span><h2>{active.length ? `${active.length} بند نشط` : 'الشجرة فارغة'}</h2></div><FolderTree size={18} /></div>
...
function CategoryNode({ category, all, state, depth, onEdit, onArchive }: { category: ExpenseCategory; all: ExpenseCategory[]; state: ReturnType<typeof useFinance>['state']; depth: number; onEdit: (c: ExpenseCategory) => void; onArchive: (c: ExpenseCategory) => void }) {
  ...
  return <div className="category-node-wrap">
    ...
    {children.length > 0 && <div className="category-children">{children.map(child => <CategoryNode key={child.id} category={child} all={all} state={state} depth={depth + 1} onEdit={onEdit} onArchive={onArchive} />)}</div>}
  </div>
}
