...
import { assetGroupOf, holdingValueSar, ownerHoldingValueSar, ownerQuantity } from '../domain/finance'
...
export function Assets() {
  ...
  return <div className="page-stack">
    <section className="section-intro"><div><span className="eyebrow">ASSET ≠ ACCOUNT</span><h2>الأصول والحسابات</h2><p>النقد أصل، والحساب أو الخزنة حاوية له. المالك يجيب «لمن؟»، الأصل «ماذا وكم؟»، والحساب/الحافظ «أين؟».</p></div><div className="segmented"><button className={lens === 'owner' ? 'active' : ''} onClick={() => setLens('owner')}>المالك</button><button className={lens === 'group' ? 'active' : ''} onClick={() => setLens('group')}>التصنيف</button><button className={lens === 'account' ? 'active' : ''} onClick={() => setLens('account')}>الحساب</button></div></section>
...
function kindName(kind: string) { return ({ cash: 'نقد وعملات', metal: 'ذهب وفضة ومعادن', collectible: 'مقتنيات', fund: 'صناديق', stock: 'أسهم', crypto: 'عملات رقمية', real_estate: 'عقارات', fixed_term: 'استثمارات لأجل', receivable: 'ذمم ومطالبات', other: 'أخرى' } as Record<string, string>)[kind] ?? kind }
