  ...
  return <div className="page-stack operations-page">
    ...
    <section className="action-picker">
      {actions.map(item => {
        ...
        return <button key={item.id} className={action === item.id ? 'action-choice active' : 'action-choice'} onClick={() => setAction(item.id)}><Icon size={18} /><span><strong>{item.title}</strong><small>{item.sub}</small></span></button>
      })}
    </section>

    <section className="operation-workspace">
      <div className="operation-form-panel">
        {action === 'purchase' && <PurchaseForm state={state} places={places} accounts={accounts} portfolios={portfolios} onSubmit={(input: SimplifiedPurchaseInput) => execute(() => finance.purchaseAsset(input), `تم شراء «${input.name}» بنجاح وتحديث الرصيد والتكلفة والمركز.`)} />}
      ...
      <aside className="operation-help"><WalletCards size={20} /><strong>قاعدة بسيطة</strong><p>{helpFor(action)}</p><div className="operation-rule"><CirclePlus size={15} /><span>نجاح أو فشل كل عملية يظهر كتأكيد عائم واضح، وتظهر النتيجة فورًا في شجرة المكان والحسابات.</span></div></aside>
    </section>
  </div>
}
...
type PurchaseProps = {
  ...
  onSubmit: (input: SimplifiedPurchaseInput) => void
}

function PurchaseForm({ state, places, accounts, portfolios, onSubmit }: PurchaseProps) {
  ...
  const [targetAccountId, setTargetAccount] = useState(activeAccounts(accounts, targetPlaceId).find(a => a.kind !== 'credit_card')?.id ?? '')
  ...
  useEffect(() => {
    if (!symbol.trim() || ['none', 'manual_appraisal', 'contractual'].includes(definition.quoteStrategy)) {
      setQuote(null); setQuoteState('fallback'); return
    }
    let active = true
  ...
  const input: SimplifiedPurchaseInput = {
    ...
    symbol: symbol.trim() || definition.defaultSymbol,
  ...
  const changeSourcePlace = (placeId: string) => {
    ...
    const holding = state.holdings.find(h => h.kind === 'cash' && h.accountId === account?.id && ownerQuantity(h, SELF_ID) > 0)
    setSourceHolding(holding?.id ?? '')
  }
  const changeSourceAccount = (accountId: string) => {
    setSourceAccount(accountId)
    const holding = state.holdings.find(h => h.kind === 'cash' && h.accountId === accountId && ownerQuantity(h, SELF_ID) > 0)
    setSourceHolding(holding?.id ?? '')
  }
  const changeTargetPlace = (placeId: string) => {
    setTargetPlace(placeId)
    setTargetAccount(activeAccounts(accounts, placeId).find(a => a.kind !== 'credit_card')?.id ?? '')
  }
  ...
  return <FormShell title="شراء أصل" subtitle="اختر النوع من الدليل، أدخل ما دفعته وما استلمته، والنظام يحسب Cost Basis ويحاول جلب التقييم السوقي.">
    <form className="trade-form" onSubmit={submit}>
      ...
      <div className="operation-rule"><span>الربح/الخسارة = القيمة الحالية − التكلفة التاريخية. يظهر غير محقق أثناء الاحتفاظ، ويصبح محققًا فقط عند البيع أو التسييل الحقيقي إلى نقد.</span></div>
      <button className="primary wide" type="submit" disabled={!preview || !targetAccountId}>تأكيد شراء الأصل</button>
    </form>
  </FormShell>
}

type AccountFormProps = { places: Party[]; onSubmit: (input: AddAccountInput) => void }
function AccountForm({ places, onSubmit }: AccountFormProps) {
  ...
  return <FormShell title="إضافة حساب تحت مكان" subtitle="مثال: مصرف الراجحي ← جاري شخصي / بطاقة / حساب استثماري."><form className="trade-form" onSubmit={submit}>
...
type FundsProps = { owners: Party[]; places: Party[]; accounts: Account[]; portfolios: Portfolio[]; onSubmit: (input: AddFundsInput) => void }
function FundsForm({ owners, places, accounts, portfolios, onSubmit }: FundsProps) {
  const [ownerId, setOwner] = useState(owners.find(o => o.id === SELF_ID)?.id ?? owners[0]?.id ?? '')
  const [placeId, setPlace] = useState(places[0]?.id ?? '')
  const [accountId, setAccount] = useState(activeAccounts(accounts, placeId).find(a => a.kind !== 'credit_card')?.id ?? '')
...
type ExistingProps = { owners: Party[]; places: Party[]; accounts: Account[]; portfolios: Portfolio[]; onSubmit: (input: ExistingAssetInput) => void }
function ExistingAssetForm({ owners, places, accounts, portfolios, onSubmit }: ExistingProps) {
  const [ownerId, setOwner] = useState(owners.find(o => o.id === SELF_ID)?.id ?? owners[0]?.id ?? '')
  const [placeId, setPlace] = useState(places[0]?.id ?? '')
  const [accountId, setAccount] = useState(activeAccounts(accounts, placeId).find(a => a.kind !== 'credit_card')?.id ?? '')
...
type TransferProps = { state: FinanceState; places: Party[]; accounts: Account[]; onSubmit: (input: TransferFundsInput) => void }
function TransferForm({ state, places, accounts, onSubmit }: TransferProps) {
  const [sourcePlaceId, setSourcePlace] = useState(places[0]?.id ?? '')
  const [sourceAccountId, setSourceAccount] = useState(activeAccounts(accounts, sourcePlaceId)[0]?.id ?? '')
  ...
  const changeSourcePlace = (id: string) => { setSourcePlace(id); const account = activeAccounts(accounts, id)[0]; setSourceAccount(account?.id ?? ''); setHolding(state.holdings.find(h => h.kind === 'cash' && h.accountId === account?.id)?.id ?? '') }
  const changeSourceAccount = (id: string) => { setSourceAccount(id); setHolding(state.holdings.find(h => h.kind === 'cash' && h.accountId === id)?.id ?? '') }
  ...
  return <FormShell title="نقل أموال" subtitle="نفس العملة تنتقل من حساب إلى حساب. لا دخل ولا مصروف ولا P/L."><form className="trade-form" onSubmit={submit}>
    <div className="divider">من</div><div className="field-grid"><Field label="المكان"><select value={sourcePlaceId} onChange={e => changeSourcePlace(e.target.value)}>{places.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}</select></Field><Field label="الحساب"><select value={sourceAccountId} onChange={e => changeSourceAccount(e.target.value)}>{activeAccounts(accounts, sourcePlaceId).map(a => <option key={a.id} value={a.id}>{a.name}</option>)}</select></Field></div>
...
type PortfolioProps = { owners: Party[]; portfolios: Portfolio[]; onSubmit: (input: CreatePortfolioInput) => void }
function PortfolioForm({ owners, portfolios, onSubmit }: PortfolioProps) {
  const [ownerId, setOwner] = useState(owners.find(o => o.id === SELF_ID)?.id ?? owners[0]?.id ?? '')
  const [name, setName] = useState('')
  ...
  return <FormShell title="إنشاء محفظة" subtitle="المحفظة WHY وليست مكانًا ولا حسابًا."><form className="trade-form" onSubmit={submit}><div className="field-grid"><Field label="اسم المحفظة"><input value={name} onChange={e => setName(e.target.value)} /></Field><Field label="المالك"><select value={ownerId} onChange={e => setOwner(e.target.value)}>{owners.map(o => <option key={o.id} value={o.id}>{o.name}</option>)}</select></Field></div><div className="field-grid"><Field label="السلوك"><select value={profile} onChange={e => setProfile(e.target.value as PortfolioProfile)}>{profiles.map(x => <option key={x.value} value={x.value}>{x.label}</option>)}</select></Field><Field label="محفظة أم"><select value={parentId} onChange={e => setParent(e.target.value)}><option value="">جذرية</option>{portfolios.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}</select></Field></div><Field label="الغرض"><input value={purpose} onChange={e => setPurpose(e.target.value)} /></Field><Field label="الهدف بالريال"><input value={target} onChange={e => setTarget(e.target.value)} inputMode="decimal" /></Field><button className="primary wide" type="submit">إنشاء المحفظة</button></form></FormShell>
}

type AllocateProps = { state: FinanceState; owners: Party[]; holdings: Holding[]; portfolios: Portfolio[]; onSubmit: (input: AllocateToPortfolioInput) => void }
function AllocateForm({ state, owners, holdings, portfolios, onSubmit }: AllocateProps) {
  const [ownerId, setOwner] = useState(owners.find(o => o.id === SELF_ID)?.id ?? owners[0]?.id ?? '')
  ...
  return <FormShell title="تخصيص لمحفظة" subtitle="تغيير الغرض فقط؛ الأصل يبقى في مكانه وحسابه."><form className="trade-form" onSubmit={submit}><Field label="المالك"><select value={ownerId} onChange={e => { setOwner(e.target.value); setHolding('') }}>{owners.map(o => <option key={o.id} value={o.id}>{o.name}</option>)}</select></Field><Field label="الأصل"><select value={holdingId} onChange={e => setHolding(e.target.value)}><option value="">اختر الأصل</option>{eligible.map(h => <option key={h.id} value={h.id}>{h.name} — حر {availableQuantity(state, h.id, ownerId).toLocaleString('ar-SA')} {h.nativeUnit}</option>)}</select></Field><div className="field-grid"><Field label="الكمية"><input value={quantity} onChange={e => setQuantity(e.target.value)} inputMode="decimal" /><small>المتاح: {free.toLocaleString('ar-SA')} {holding?.nativeUnit}</small></Field><Field label="المحفظة"><select value={portfolioId} onChange={e => setPortfolio(e.target.value)}>{portfolios.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}</select></Field></div><button className="primary wide" type="submit">تخصيص</button></form></FormShell>
}
