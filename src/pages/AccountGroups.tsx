...
export function AccountGroups() {
  ...
  return <div className="page-stack">
    ...
    {accountModalOpen && editingAccount && <div className="modal-backdrop" onMouseDown={e => { if (e.target === e.currentTarget) closeAccountEdit() }}><section className="expense-modal category-modal" role="dialog" aria-modal="true" aria-label="تعديل الحساب"><button className="modal-close" onClick={closeAccountEdit} aria-label="إغلاق"><X size={18}/></button><div className="panel-head"><div><span>تعديل بيانات الحساب</span><h2>تعديل «{editingAccount.name}»</h2><span>التعديل يغيّر بيانات الحساب التنظيمية فقط. معرف الحساب والحيازات وCost Basis والـLedger تبقى كما هي.</span></div></div><form className="trade-form" onSubmit={submitAccount}>
      ...
      <div className="operation-rule"><span>ملاحظة: إذا كان الحساب يحتوي أرصدة فلن يسمح النظام بتحويله من/إلى «بطاقة ائتمان»، لأن ذلك يغير المعنى المالي للحساب وليس مجرد وصفه.</span></div>
      <button className="primary wide" type="submit">حفظ تعديلات الحساب</button>
    </form></section></div>}
  </div>
}

function AccountOrganizerRow({ account, groups, state, onMove, onEdit }: { account: Account; groups: AccountGroup[]; state: ReturnType<typeof useFinance>['state']; onMove: (account: Account, groupId: string) => void; onEdit: (account: Account) => void }) {
  ...
  return <div className="account-organizer-row"><div><WalletCards size={16}/><span><strong>{account.name}</strong><small>{accountKindName(account.kind)}{account.currency ? ` • ${account.currency}` : ''} • {holdings.length} أصول/أرصدة • {money.format(value)} ر.س</small></span></div><div className="account-organizer-actions"><select value={account.groupId ?? ''} onChange={e => onMove(account, e.target.value)}><option value="">بدون مجموعة</option>{groups.map(g => <option key={g.id} value={g.id}>{g.name}</option>)}</select><button className="account-edit-button" title="تعديل الحساب" aria-label={`تعديل ${account.name}`} onClick={() => onEdit(account)}><Pencil size={14}/></button></div></div>
}

function GroupNode({ group, all, state, depth, expanded, onToggle, onEdit, onArchive, onMove, onEditAccount }: { group: AccountGroup; all: AccountGroup[]; state: ReturnType<typeof useFinance>['state']; depth: number; expanded: Set<string>; onToggle: (id: string) => void; onEdit: (g: AccountGroup) => void; onArchive: (g: AccountGroup) => void; onMove: (account: Account, groupId: string) => void; onEditAccount: (account: Account) => void }) {
