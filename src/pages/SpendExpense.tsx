...
export function SpendExpense() {
  ...
  const submit = (event: FormEvent) => {
    ...
    try {
      ...
      setTitle('')
      setNote('')
      setMessage({ type: 'ok', text: portfolioId ? 'تم تسجيل المصروف وخصمه فعليًا من الحساب واستهلاكه من رصيد المحفظة.' : 'تم تسجيل المصروف وخصمه من السيولة الحرة دون ربطه بمحفظة.' })
    } catch (error) {
      setMessage({ type: 'error', text: error instanceof Error ? error.message : 'تعذر تسجيل المصروف' })
    }
  }
  ...
  return <div className="page-stack">
    <section className="section-intro">
      <div><span className="eyebrow">EXPENSE / WHAT + WHERE + WHY</span><h2>تسجيل مصروف</h2><p>اختر بند الصرف ليصف «على ماذا؟»، ومصدر الدفع ليصف «من أين خرج؟»، والمحفظة اختيارية لتصف «من أي غرض/مخصص؟».</p></div>
    </section>
    ...
    <section className="operation-workspace">
      <div className="operation-form-panel">
        <div className="panel operation-form">
          <div className="panel-head"><div><span>حركة مالية فعلية</span><h2>بيانات عملية الصرف</h2><span>المصروف يقلل الثروة. ربطه بمحفظة يستهلك أيضًا من تخصيص المحفظة، لكنه لا يغير حقيقة الحساب الذي دفع.</span></div><ReceiptText size={18} /></div>
