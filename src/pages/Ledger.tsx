import { ArrowDownLeft, ArrowLeftRight, ArrowUpRight, Boxes, FolderTree, History, Pencil, RefreshCcw, Repeat2, ShieldAlert, SlidersHorizontal, UserRound, WalletCards, X } from 'lucide-react'
...
export function Ledger() {
  ...
  return <div className="page-stack">
    <section className="section-intro"><div><span className="eyebrow">LOGICAL TRANSACTIONS + AUDIT</span><h2>الحركات</h2><p>يمكن تصحيح البيانات الوصفية والتصنيفية للحركة مع الاحتفاظ بكل نسخة سابقة. أما الحقول التي تغيّر الرصيد أو Cost Basis فتحتاج إعادة إسقاط مالي ولا تُعدل بصمت.</p></div><div className="lock-note"><History size={16}/> محفوظة المراجعات</div></section>
...
function TransactionEditModal({ tx, state, onClose, onSave }: { tx: LedgerTransaction; state: ReturnType<typeof useFinance>['state']; onClose: () => void; onSave: (input: Parameters<ReturnType<typeof useFinance>['reviseTransaction']>[0]) => void }) {
  ...
  return <div className="modal-backdrop" onMouseDown={e => { if (e.target === e.currentTarget) onClose() }}><section className="expense-modal transaction-edit-modal" role="dialog" aria-modal="true" aria-label="تعديل حركة"><button className="modal-close" onClick={onClose} aria-label="إغلاق"><X size={18}/></button><div className="panel-head"><div><span>LOGICAL TRANSACTION • v{tx.version}</span><h2>تعديل الحركة</h2><span>سيبقى نفس معرّف الحركة، وتُحفظ النسخة الحالية في سجل المراجعات قبل اعتماد التعديل.</span></div></div><form className="trade-form" onSubmit={submit}>
    ...
    <button className="primary wide" type="submit">حفظ كمراجعة v{tx.version + 1}</button>
  </form></section></div>
}
