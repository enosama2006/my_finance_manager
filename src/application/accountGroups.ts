...
/** Canonical operation: changing placement is organizational only. */
export { moveAssetToGroup }

// ---------------------------------------------------------------------------
// Legacy account API retained temporarily so old tests/import paths keep working.
// New UI does not create or organize accounts.
// ---------------------------------------------------------------------------
export function moveAccountToGroup(state: FinanceState, accountId: string, groupId?: string): FinanceState {
  const account = state.accounts.find(a => a.id === accountId)
  if (!account) throw new Error('الحساب القديم غير موجود')
...
export function updateAccount(state: FinanceState, input: UpdateAccountInput): FinanceState {
  ...
  if (!account) throw new Error('الحساب القديم غير موجود أو غير نشط')
  const name = input.name.trim()
