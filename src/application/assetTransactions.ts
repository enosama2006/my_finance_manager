...
function setOwnerQuantity(asset: Holding, ownerId: string, desired: number, unitCostSar?: number, increaseBasisSar?: number) {
  ...
  if (delta < -1e-9) {
    ...
    costLots = costLots.map(lot => {
      ...
      const used = Math.min(lot.quantity, remaining); remaining -= used
  ...
  if (desired > 1e-9) ownership.push({ ownerId, quantity: desired })
  return { ...asset, quantity: asset.quantity + delta, ownership, costLots }
}
...
export function setAssetOpeningBalance(state: FinanceState, input: SetAssetOpeningBalanceInput): FinanceState {
  ...
  const currentOpening = existing.reduce((sum, tx) => sum + (tx.targetQuantity ?? 0), 0)
  const currentOwner = ownerQuantity(asset, input.ownerId)
  const nonOpening = currentOwner - currentOpening
  const desiredOwner = nonOpening + input.quantity
  if (desiredOwner < -1e-9) throw new Error('لا يمكن ضبط الرصيد الافتتاحي بهذه القيمة بسبب حركات لاحقة')

  // SAR has an intrinsic SAR basis of 1. Foreign-currency historical basis is never inferred from current FX.
  const currentKnownUnitCost = ownerWeightedAverageCostSar(asset, input.ownerId)
  const unitCost = input.unitCostSar ?? currentKnownUnitCost ?? (asset.symbol.toUpperCase() === 'SAR' ? 1 : undefined)
  ...
  if (existing.length === 0) {
    const tx: LedgerTransaction = { id: id('tx'), version: 1, status: 'posted', revisions: [], at: now(), kind: 'opening', title: input.title?.trim() || `رصيد افتتاحي: ${asset.name}`, amountSar: openingReportingValueSar, costBasisSar: openingBasisSar, ownerId: input.ownerId, targetHoldingId: asset.id, targetQuantity: input.quantity, note: openingBasisSar == null ? 'حالة افتتاحية؛ القيمة الحالية مسجلة للتقرير، أما التكلفة التاريخية فغير معروفة ولم يتم افتراض سعر صرف لها.' : 'حالة افتتاحية واحدة لهذا الأصل؛ التكلفة التاريخية محفوظة مستقلة عن القيمة الحالية.' }
  ...
  const updatedTx: LedgerTransaction = { ...canonical, version: canonical.version + 1, revisions: [...canonical.revisions, { version: canonical.version, changedAt: now(), reason, snapshot: snapshot(canonical) }], title: input.title?.trim() || canonical.title, amountSar: openingReportingValueSar, costBasisSar: openingBasisSar, targetQuantity: input.quantity, note: existing.length > 1 ? `تم توحيد ${existing.length} حالات افتتاحية في سجل واحد.` : openingBasisSar == null ? `تم التصحيح: ${reason}. التكلفة التاريخية ما زالت غير معروفة.` : `تم التصحيح: ${reason}` }
...
export function createAssetWithOpening(state: FinanceState, input: CreateAssetInput): FinanceState {
  ...
  const unitCost = input.costBasisSar != null ? input.costBasisSar / quantity : (asset.symbol.toUpperCase() === 'SAR' ? 1 : undefined)
...
export function addIncomeToAsset(state: FinanceState, input: AddAssetIncomeInput): FinanceState {
  ...
  const updated = setOwnerQuantity(asset, input.ownerId, current + input.quantity, unitCost, basis)
  const tx: LedgerTransaction = { id: id('tx'), version: 1, status: 'posted', revisions: [], at: now(), kind: 'income', title: input.title?.trim() || `دخل إلى ${asset.name}`, amountSar: round2(input.quantity * unitCost), costBasisSar: basis, ownerId: input.ownerId, targetHoldingId: asset.id, targetQuantity: input.quantity, note: input.note?.trim() || 'دخل أضافه المستخدم إلى الأصل النقدي.' }
...
export function transferBetweenAssets(state: FinanceState, input: TransferBetweenAssetsInput): FinanceState {
  ...
  const nextSource = setOwnerQuantity(source, input.ownerId, sourceOwned - input.quantity, sourceCost)
  const targetOwned = ownerQuantity(target, input.ownerId)
  const nextTarget = setOwnerQuantity(target, input.ownerId, targetOwned + input.quantity, sourceCost, transferredBasis)
  const tx: LedgerTransaction = { id: id('tx'), version: 1, status: 'posted', revisions: [], at: now(), kind: 'real_transfer', title: `نقل ${source.symbol}: ${source.name} ← ${target.name}`, amountSar: round2(input.quantity * source.marketPriceSar), costBasisSar: transferredBasis ?? null, ownerId: input.ownerId, sourceHoldingId: source.id, targetHoldingId: target.id, sourceQuantity: input.quantity, targetQuantity: input.quantity, note: input.note?.trim() || (transferredBasis == null ? 'نقل بين أصلين نقديين بنفس العملة؛ التكلفة التاريخية للمصدر غير معروفة فبقيت غير معروفة.' : 'نقل بين أصلين نقديين بنفس العملة؛ لا يحقق ربحًا أو خسارة.') }
