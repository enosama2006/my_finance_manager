...
interface FinanceContextValue {
  ...
  // Legacy compatibility only; new presentation does not depend on Accounts.
...
function materializeCategoryNecessity(categories: ExpenseCategory[]): ExpenseCategory[] {
  const byId = new Map(categories.map(c => [c.id, c])); const resolving = new Set<string>(); const resolved = new Map<string, ExpenseCategory>()
  const resolve = (category: ExpenseCategory): ExpenseCategory => {
    ...
    resolving.add(category.id); const parent = byId.get(category.parentId); const parentResolved = parent ? resolve(parent) : undefined; resolving.delete(category.id)
    const next = parentResolved?.defaultNecessity ? { ...category, defaultNecessity: parentResolved.defaultNecessity } : category; resolved.set(category.id, next); return next
  }
  return categories.map(resolve)
}

function migrateLegacyPlacesToGroups(state: FinanceState): FinanceState {
  let accountGroups = [...(state.accountGroups ?? [])]; let accounts = [...state.accounts]; const selfId = state.parties.find(p => p.type === 'self')?.id
  for (const account of accounts) {
    if (account.groupId || account.custodianId === selfId) continue
    const legacy = state.parties.find(p => p.id === account.custodianId && p.type !== 'self' && p.type !== 'person'); if (!legacy) continue
...
function normalize(state: FinanceState): FinanceState {
  const withGroups = migrateLegacyPlacesToGroups(state); const withInputs = hydrateTransactionUserInputs(withGroups); const migrated = flattenLegacyAccountsToAssets(withInputs)
...
export function FinanceProvider({ children }: { children: ReactNode }) {
  ...
  const value = useMemo<FinanceContextValue>(() => ({
    ...
    deleteAsset: (assetId, reason) => {
      ...
      if (origin && !downstream) persist(voidAssetPurchase(state, origin.id, reason)); else persist(archiveAssetCorrection(state, assetId, reason))
    },
