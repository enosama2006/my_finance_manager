...
function debitOwnerHolding(holding: Holding, ownerId: string, quantity: number): Holding {
  ...
  return { ...holding, quantity: round2(holding.quantity - quantity), ownership: holding.ownership.map(s => s.ownerId === ownerId ? { ...s, quantity: round2(s.quantity - quantity) } : s).filter(s => s.quantity > 0), costLots: reduceOwnerLots(holding, ownerId, quantity) }
}
...
function consumePortfolioFunding(state: FinanceState, input: SimplifiedPurchaseInput): FinanceState {
  ...
  const allocated = state.portfolioSlices.filter(s => s.portfolioId === input.portfolioId && s.holdingId === input.sourceHoldingId && s.ownerId === input.ownerId).reduce((sum, s) => sum + s.quantity, 0)
  ...
  return { ...state, portfolioSlices: state.portfolioSlices.map(slice => {
    ...
    const used = Math.min(slice.quantity, remaining); remaining = round2(remaining - used)
...
export function purchaseAssetSimplified(state: FinanceState, input: SimplifiedPurchaseInput): FinanceState {
  ...
  const holdingId = id('holding'); const positionId = id('pos')
  ...
  const holding: Holding = {
    ...
    quantity: input.quantity, marketPriceSar, costLots: [{ id: id('lot'), ownerId: input.ownerId, quantity: input.quantity, unitCostSar: preview.effectiveUnitCostSar, acquiredAt: now() }],
    valuationMethod: input.marketQuote ? 'market_quote' : valuationMethodForFallback(def.id), valuationSource: input.marketQuote?.source ?? 'acquisition-cost-fallback', valuedAt: input.marketQuote?.asOf ?? now(),
    groupId: groupId || undefined, location: input.location?.trim() || undefined, ownership: [{ ownerId: input.ownerId, quantity: input.quantity }], performanceRole: def.defaultPerformanceRole, positionId,
  ...
  next = { ...next, holdings: [...next.holdings, holding], positions: [...(next.positions ?? []), {
    id: positionId, name: input.name.trim(), ownerId: input.ownerId, portfolioId: input.portfolioId, holdingIds: [holdingId], openedAt: now(), status: 'open', performanceRole: def.defaultPerformanceRole,
    initialCostBasisSar: preview.totalCostBasisSar, realizedGainLossSar: 0, note: 'التكلفة محفوظة تاريخيًا. فرق القيمة السوقية يبقى غير محقق حتى البيع/التسييل الحقيقي.',
  }] }
  ...
  const tx: LedgerTransaction = {
    id: id('tx'), version: 1, status: 'posted', revisions: [], at: now(), kind: 'asset_purchase', title: `شراء ${input.name.trim()}`, amountSar: preview.totalCostBasisSar, ownerId: input.ownerId,
    sourceHoldingId: source.id, targetHoldingId: holdingId, sourceQuantity: input.amountPaid, targetQuantity: input.quantity, feesSar: preview.extraCostsSar || undefined, positionId, portfolioId: input.portfolioId,
    userInput: { kind: 'asset_purchase', sourceAccountId: source.accountId, sourceHoldingId: source.id, ownerId: input.ownerId, amountPaid: input.amountPaid, targetGroupId: groupId || undefined, targetAccountId: input.targetAccountId, assetTypeId: def.id, name: input.name.trim(), symbol, quantity: input.quantity, extraCostsSar: preview.extraCostsSar || undefined, portfolioId: input.portfolioId, location: input.location?.trim() || undefined, marketUnitPriceSar: marketPriceSar, marketSource: input.marketQuote?.source ?? holding.valuationSource },
    note: input.marketQuote ? `التقييم السوقي جُلب من ${input.marketQuote.source}. الربح/الخسارة الحالية غير محققة.` : 'لم يتوفر Quote سوقي لحظة الشراء؛ بدأ التقييم مؤقتًا من تكلفة الاقتناء وسيُحدّث عند توفر مزود سعر.',
  }
