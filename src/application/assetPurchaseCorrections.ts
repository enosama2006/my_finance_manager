...
function addOwnerQuantity(holding: Holding, ownerId: string, quantity: number, unitCostSar: number, acquiredAt: string): Holding {
  const current = ownerQuantity(holding, ownerId)
  const ownership = holding.ownership.some(s => s.ownerId === ownerId)
    ...
    : [...holding.ownership, { ownerId, quantity }]
  return {
    ...holding,
    quantity: round2(holding.quantity + quantity),
    ownership,
    costLots: [...holding.costLots, { id: id('lot'), ownerId, quantity, unitCostSar, acquiredAt }],
  }
}
...
function assertPurchaseCanBeReprojected(state: FinanceState, tx: LedgerTransaction) {
  ...
  if (Math.abs(target.quantity - tx.targetQuantity) > 0.000001 || Math.abs(targetOwned - tx.targetQuantity) > 0.000001 || otherOwners.length) {
    throw new Error('لا يمكن تصحيح الشراء مباشرة لأن كمية/ملكية الأصل تغيرت لاحقًا')
  }

  if (tx.portfolioId) {
    throw new Error('تصحيح شراء مرتبط بمحفظة يحتاج سجل Projection تفصيلي لاستعادة تمويل المحفظة بدقة. لن نخمن الأثر.')
  }

  if (tx.positionId) {
    const position = (state.positions ?? []).find(p => p.id === tx.positionId)
    if (!position || position.status !== 'open' || position.realizedGainLossSar !== 0 || position.holdingIds.length !== 1 || position.holdingIds[0] !== target.id) {
      throw new Error('المركز المرتبط بالشراء تغير لاحقًا؛ يلزم Replay متسلسل بدل تعديل مباشر')
    }
  }
}
...
function reversePurchaseProjection(state: FinanceState, tx: LedgerTransaction): FinanceState {
  ...
  return {
    ...state,
    holdings: state.holdings.filter(h => h.id !== targetId).map(h => h.id === source.id ? restoredSource : h),
    portfolioSlices: state.portfolioSlices.filter(s => s.holdingId !== targetId),
    positions: (state.positions ?? []).filter(p => !positionId || p.id !== positionId),
    ledger: state.ledger.filter(item => item.id !== tx.id),
  }
}
...
export function correctAssetPurchase(state: FinanceState, input: CorrectAssetPurchaseInput): FinanceState {
  ...
  const marketQuote: MarketQuote | null = input.marketUnitPriceSar && input.marketUnitPriceSar > 0 ? {
    unitPriceSar: input.marketUnitPriceSar,
    source: input.marketSource?.trim() || 'user-correction',
    asOf: at.toISOString(),
    isLive: false,
    note: 'قيمة مصححة يدويًا ضمن مراجعة حركة شراء.',
  } : null
  const purchaseInput: SimplifiedPurchaseInput = {
    ...
    extraCostsSar: input.extraCostsSar,
  ...
  const correctedTx: LedgerTransaction = {
    ...generated,
    id: current.id,
    version: current.version + 1,
    revisions: [...current.revisions, revision],
    at: at.toISOString(),
    title: input.title?.trim() || `شراء ${input.name.trim()}`,
    note: input.note?.trim() || generated.note,
  }
  ...
  replayed = {
    ...
    holdings: replayed.holdings.map(h => h.id === generated.targetHoldingId ? {
      ...h,
      costLots: h.costLots.map(lot => ({ ...lot, acquiredAt: at.toISOString() })),
      valuedAt: marketQuote?.asOf ?? h.valuedAt,
    } : h),
    positions: (replayed.positions ?? []).map(position => position.id === generated.positionId ? { ...position, openedAt: at.toISOString() } : position),
  }
  return replayed
}
...
export function voidAssetPurchase(state: FinanceState, transactionId: string, reason: string): FinanceState {
  ...
  const voided: LedgerTransaction = {
    ...current,
    status: 'voided',
    version: current.version + 1,
    revisions: [...current.revisions, revision],
  ...
  return { ...base, ledger: [voided, ...base.ledger] }
}
