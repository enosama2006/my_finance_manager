...
export function voidTransaction(state:FinanceState,transactionId:string,reason:string):FinanceState{
 ...
 if(tx.kind==='income'){
  if(!tx.targetHoldingId||!tx.targetQuantity)throw new Error('بيانات الدخل غير كافية للعكس');const target=asset(state,tx.targetHoldingId);const next=remove(target,tx.ownerId,tx.targetQuantity);return mark(state,tx,why,state.holdings.map(h=>h.id===target.id?next:h))
 }
 if(tx.kind==='expense'){
  if(!tx.sourceHoldingId||!tx.sourceQuantity)throw new Error('بيانات المصروف غير كافية للعكس');const source=asset(state,tx.sourceHoldingId);const unit=tx.sourceQuantity>0?tx.amountSar/tx.sourceQuantity:source.marketPriceSar;const next=add(source,tx.ownerId,tx.sourceQuantity,unit);let slices=state.portfolioSlices;if(tx.portfolioId){const found=slices.find(s=>s.portfolioId===tx.portfolioId&&s.holdingId===source.id&&s.ownerId===tx.ownerId);slices=found?slices.map(s=>s.id===found.id?{...s,quantity:round2(s.quantity+tx.sourceQuantity!)}:s):[...slices,{id:id('slice'),portfolioId:tx.portfolioId,holdingId:source.id,ownerId:tx.ownerId,quantity:tx.sourceQuantity}]};return mark(state,tx,why,state.holdings.map(h=>h.id===source.id?next:h),slices)
 }
 if(tx.kind==='real_transfer'){
  if(!tx.sourceHoldingId||!tx.targetHoldingId||!tx.sourceQuantity||!tx.targetQuantity)throw new Error('بيانات النقل غير كافية للعكس');const source=asset(state,tx.sourceHoldingId);const target=asset(state,tx.targetHoldingId);const targetNext=remove(target,tx.ownerId,tx.targetQuantity);const unit=tx.sourceQuantity>0?tx.amountSar/tx.sourceQuantity:source.marketPriceSar;const sourceNext=add(source,tx.ownerId,tx.sourceQuantity,unit);return mark(state,tx,why,state.holdings.map(h=>h.id===source.id?sourceNext:h.id===target.id?targetNext:h))
 }
 if(tx.kind==='reconciliation'){
  if(tx.targetHoldingId&&tx.targetQuantity){const target=asset(state,tx.targetHoldingId);const next=remove(target,tx.ownerId,tx.targetQuantity);return mark(state,tx,why,state.holdings.map(h=>h.id===target.id?next:h))}
  if(tx.sourceHoldingId&&tx.sourceQuantity){const source=asset(state,tx.sourceHoldingId);const unit=tx.sourceQuantity>0?tx.amountSar/tx.sourceQuantity:source.marketPriceSar;const next=add(source,tx.ownerId,tx.sourceQuantity,unit);return mark(state,tx,why,state.holdings.map(h=>h.id===source.id?next:h))}
